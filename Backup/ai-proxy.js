...
app.post('/v1/models/analyze', async (req, res) => {
  ...
  try {
    ...
    // Step 1: Query SQLite cache for items analyzed in past sessions
...
`;
    ...
    // Step 4: Dispatch payload to active network driver
    ...
    // --- HELPER: KEY NORMALIZATION FUNCTION ---
    // Maps messy or capitalized LLM keys into your expected database schema keys
    const normalizeKeys = (obj) => {
      ...
      for (const key of Object.keys(obj)) {
        ...
        if (lowerKey === 'transaction_id' || lowerKey === 'txn_ref' || lowerKey === 'id') {
          ...
          normalized[key] = obj[key]; // Keep original properties if outside core list
        }
      }
      return normalized;
    };

    // --- ENHANCED SELF-HEALING DEFENSIVE NORMALIZATION LAYER ---
    if (Array.isArray(rawParsedResult)) {
      // Case A: Ideal output format (Array)
      ...
      // Pre-normalize the root object so single object validation isn't case-sensitive
      const normalizedRoot = normalizeKeys(rawParsedResult);

      // Case B: Array is buried inside an object property wrapper (e.g., { "analysis": [...] })
      ...
      // Case C: Single standalone flat log entry object (Catches your TXN_REF case anomaly)
      ...
      // Case D: Map/Lookup format where key names represent ID mapping (e.g., { "TXN-2001": { ... } })
      else {
    ...
    // Double-check final shape status
    ...
    // Step 5: Write newly inferred evaluations into SQLite cache memory bank
    for (const evaluatedRisk of parsedLLMArray) {
      if (!evaluatedRisk) continue;

      const rawTxnId = evaluatedRisk.transaction_id;
      if (!rawTxnId) {
        console.warn("⚠️ Skipping unidentifiable risk block lacking transaction identifier:", evaluatedRisk);
        continue;
      }

      // Clean dollar string values if returned with raw characters (e.g. "-$2400" -> -2400)
      let cleanAmount = 0;
